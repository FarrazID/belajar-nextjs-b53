...
export default function ConfirmationModal({ id }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // const response = await fetch(`https://paace-f178cafcae7b.nevacloud.io/api/notes/${selectedId}`);
  ...
  const handleDelete = async (event) => {
    ...
    const result = await mutate({
      url: `/api/notes/delete/${selectedId}`,
      // url: `https://paace-f178cafcae7b.nevacloud.io/api/notes/delete/${selectedId}`,
      method: 'DELETE'
    })
    console.log(id);
    if (result?.success) {
